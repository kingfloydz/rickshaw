...
class TowForceAction(ActionTerm):
  ...
  @property
  def current_force_w(self) -> torch.Tensor:
    quat_w = self._entity.data.root_link_quat_w[:, None, :].expand(-1, 2, -1)
    return quat_apply(
      quat_w.reshape(-1, 4), self._current_force_b.reshape(-1, 3)
    ).view_as(self._current_force_b)

  @property
  def hitch_pos_w(self) -> torch.Tensor:
  ...
  def apply_actions(self) -> None:
    force_w = self.current_force_w
    root_com_w = self._entity.data.root_com_pos_w[:, None, :]
    torque_w = torch.cross(self.hitch_pos_w - root_com_w, force_w, dim=-1).sum(dim=1)
